...
`rarexsec-training-pool` executable provides the same structured output layout
as the general runner but restricts the stored branches to a training
pool-friendly subset. Usage mirrors the main runner, except that an output file
is mandatory:
...
so that every dataset contributes consistently to the generated training pool.
