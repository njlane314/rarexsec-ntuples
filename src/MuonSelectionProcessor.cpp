...
ROOT::RVec<bool> makeMuonMask(const ROOT::RVec<float> &scores, const ROOT::RVec<float> &llr,
    ...
    for (std::size_t idx = 0; idx < track_count; ++idx) {
        mask.emplace_back(selc::isMuonCandidate(scores[idx], llr[idx], lengths[idx], dists[idx], gens[idx],
                                                start_x[idx], start_y[idx], start_z[idx], end_x[idx], end_y[idx],
                                                end_z[idx], plane_hits_u[idx], plane_hits_v[idx], plane_hits_y[idx]));
    }
    return mask;
}
