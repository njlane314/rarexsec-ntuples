...
ROOT::RDF::RNode ReconstructionProcessor::process(ROOT::RDF::RNode df, SampleOrigin st) const {
    auto fiducial_df = df.Define(
        "in_reco_fiducial",
        [](float x, float y, float z) { return ::proc::isInFiducialVolumeWithGap(x, y, z); },
        {"reco_neutrino_vertex_sce_x", "reco_neutrino_vertex_sce_y", "reco_neutrino_vertex_sce_z"});

    auto gen2_df = ::proc::ensureGenerationCount(fiducial_df, "n_pfps_gen2", 2u);
    auto gen3_df = ::proc::ensureGenerationCount(gen2_df, "n_pfps_gen3", 3u);
    auto trigger_df = ::proc::ensureSoftwareTrigger(gen3_df, st);
    ...
    auto quality_df = trigger_df.Define(
        ...
        [st](float pe_beam, float pe_veto, int num_slices, float topo, float x, float y, float z, float contained_frac,
            ...
            const bool dataset_gate = ::proc::passesDatasetGate(st, pe_beam, pe_veto, true);
            const bool basic_reco = ::proc::isSingleGoodSlice(num_slices, topo);
            const bool fv = ::proc::isInFiducialVolumeWithGap(x, y, z);
            const bool slice_quality = ::proc::passesSliceQuality(contained_frac, associated_frac);
