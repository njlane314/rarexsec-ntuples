...
ROOT::RDF::RNode ReconstructionProcessor::process(ROOT::RDF::RNode df, SampleOrigin st) const {
    auto fiducial_df = df.Define(
        "in_reco_fiducial",
        [](float x, float y, float z) { return selection::isInFiducialVolumeWithGap(x, y, z); },
        {"reco_neutrino_vertex_sce_x", "reco_neutrino_vertex_sce_y", "reco_neutrino_vertex_sce_z"});

    auto gen2_df = selection::ensureGenerationCount(fiducial_df, "n_pfps_gen2", 2u);
    auto gen3_df = selection::ensureGenerationCount(gen2_df, "n_pfps_gen3", 3u);
    auto trigger_df = selection::ensureSoftwareTrigger(gen3_df, st);
