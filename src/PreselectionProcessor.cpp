...
ROOT::RDF::RNode PreselectionProcessor::process(ROOT::RDF::RNode df, SampleOrigin st) const {
    auto base_df = ::proc::ensureGenerationCount(df, "n_pfps_gen2", 2u);
    auto trigger_df = ::proc::ensureSoftwareTrigger(base_df, st);

    auto pre_df = trigger_df.Define(
        "pass_pre",
        [st](float pe_beam, float pe_veto, bool swtrig) {
            return ::proc::passesDatasetGate(st, pe_beam, pe_veto) && swtrig;
        },
        {"optical_filter_pe_beam", "optical_filter_pe_veto", "software_trigger"});

    auto flash_df = pre_df.Define("pass_flash", ::proc::isSingleGoodSlice,
                                  {"num_slices", "topological_score"});

    auto fv_df = flash_df.Define("pass_fv", ::proc::isInFiducialVolumeWithGap,
    ...
    auto final_df = fv_df.Define("pass_mu", "n_muons_tot > 0")
                         .Define("pass_topo", ::proc::passesSliceQuality,
